...
//                 <div><img src="https://cdn-icons-png.flaticon.com/512/2764/2764453.png" style={{width:150,height:150}}/>
...
import { useParams } from "react-router-dom"; // Use useParams to get the patient_id from the URL

const ReviewSup = () => {
    const { patient_id } = useParams(); // Get patient_id from URL params
    const [patientDetails, setPatientDetails] = useState({});
    const [therapyTile,setTherapyTile]=useState();
  
    useEffect(() => {
        const fetchPatientDetails = async () => {
            try {
                const response = await axios.get(`http://localhost:8080/api/session/${patient_id}`);
                const details = response.data; // Correctly use response.data
                setPatientDetails(details);
                // setTherapyTile
            } catch (error) {
                console.error('Error fetching patient details:', error);
                    ...
                    <div className="bg" style={{ display: "flex" }}>
                        <div style={{ alignContent: "center", marginLeft: 90, marginTop: 30 }}>
                            <div>
                                ...
                                <strong style={{ fontSize: 30, marginLeft: 20 }}>{patientDetails.patient_name}</strong>
                            </div>
                            <div>
                                <img src="https://cdn-icons-png.flaticon.com/512/2764/2764453.png" style={{ width: 150, height: 150 }} />
                                <strong style={{ fontSize: 30 }}>{patientDetails.therapist_name}</strong>
                            </div>
                        </div>
                        ...
                        <div style={{ marginLeft: 120, margin: 50 }}>
                            ...
                            <div style={{ display: "flex", alignItems: "center" }}>
                                ...
                                {title.title && title.title.map((t, idx) => (
                                  <h1  key={idx} style={{ margin: "0 0 0 10px", fontSize: "20px" }}>{t}</h1>
